...
export async function deployCommands({ guildId }: DeployCommandsProps) {
  try {
    ...
    if (guildId) {
      await rest.put(Routes.applicationGuildCommands(config.bot.DISCORD_CLIENT_ID, guildId), {
        body: commandsData,
      });
      console.log(`✅ Befehle für Guild ${guildId} deployed.`);
    } else {
      // 🌍 Global deployen
      await rest.put(Routes.applicationCommands(config.bot.DISCORD_CLIENT_ID), {
        body: commandsData,
      });
