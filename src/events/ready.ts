  ...
  client.once('ready', async () => {
    try {
      ...
      if (config.env.DEBUG_MODE) {
        log.debug('🛠️ Debug-Mode ist aktiv – zusätzliche Logs erlaubt.');
      }

      // 🚀 Slash-Commands Deployment
      if (config.env.DEPLOY_COMMANDS === true) {
        if (config.bot.DISCORD_GUILD_ID) {
          await deployCommands({
            guildId: config.bot.DISCORD_GUILD_ID,
          });
          log.success('✅ Slash-Commands (Guild) erfolgreich deployed.');
        } else {
          await deployCommands({}); // Globale Commands
          log.success('✅ Slash-Commands (Global) erfolgreich deployed.');
        }
      } else {
        log.debug('⚡ Slash-Commands Deployment wurde übersprungen (DEPLOY_COMMANDS=false).');
      }
    } catch (error) {
